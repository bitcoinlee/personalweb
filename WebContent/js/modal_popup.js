...
/*Fade in - Fade out approach - Sotos Papazerveas*/
function fadeIn(el,i, callback) {
	i = i + 0.01;
    el.style.opacity = i;
    if (i<1){setTimeout(function(){fadeIn(el,i, callback);}, 5);}
    else{
    	callback(); 
    }
}
...
Popup.show = function (divObject, referenceObject, position, options, modal) {
    ...
    popup.show(); //first show and then fade in
    
    /*Fade in - Sotos Papazerveas*/
    var i = 0;
    var el = document.getElementById(divObject);
    fadeIn(el,i , function(){/*console.log(divObject + " faded in");*/
    	
    	a = document.createElement('a'); //add Close href
        a.href =  '#';
        a.setAttribute('class', 'close');
        a.setAttribute('onClick', 'Popup.hide(this.parentElement.id)');
        el.appendChild(a);
...
/*Close modal popups on Esc - Sotos Papazerveas*/
...
/* Fade out - Sotos Papazerveas */
function fadeOut(el, i, callback) {
	i = i - 0.05;
	el.style.opacity = i;
	if (i > 0) {
		setTimeout(function() {
			fadeOut(el, i, callback);
		}, 5);
	} else {
		callback();
	}
...
/* Close all modal popups - Sotos Papazerveas */
Popup.CloseEmAll = function() {
	for ( var i in Popup.objects) {
		var p = Popup.objects[i];
		if (p.modal) {
			...
			/* Fade out - Sotos Papazerveas */
			var i = 1;
			fadeOut(p.div, i, function() {
				p.hide();
				var a = p.div.getElementsByClassName("close"); //remove Close href
...
Popup.prototype.addScreen = function () {
    ...
    var outofbounds = 2; //scrollbar correction 
    
    this.screen.style.width = Screen.getDocumentWidth() - outofbounds + "px";
    this.screen.style.height = Screen.getDocumentHeight() -outofbounds + "px";
    this.screenIframeShim.style.width = Screen.getDocumentWidth() - outofbounds + "px";
    this.screenIframeShim.style.height = Screen.getDocumentHeight() -outofbounds + "px";
